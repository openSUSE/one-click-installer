...
RepositoryWidget::RepositoryWidget(PackageBackend *backend, int index, OCI::Repository *repo, QObject *parent )
{
    ...
    RepositoryMetadata meta( repo->url().toStdString() );
    ...
    m_id = new QLabel( QString( "<b>ID:</b> %1" ).arg( QString::fromStdString( meta.id() ) ) );
    m_fingerprint = new QLabel( QString( "<b>Fingerprint:</b> %1" ).arg(  QString::fromStdString( meta.fingerprint() )  ) );
    m_created = new QLabel( QString( "<b>Created:</b> %1" ).arg( QString::fromStdString( meta.created() ) ) );
    m_expires = new QLabel( QString( "<b>Expires :</b> %1" ).arg( QString::fromStdString( meta.expires() ) ) );
