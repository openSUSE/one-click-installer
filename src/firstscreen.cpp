...
FirstScreen::FirstScreen( PackageBackend *backend, QString *tmpFileName, const QString& filename, QObject *parent )
{
    ...
    foreach( OCI::Repository *iter, m_repos) {
        ...
        foreach( OCI::Package *iter, m_packages ) {
            ...
            QObject::connect( packDetails, SIGNAL( checkboxToggled( bool ) ), this, SLOT( checkPackageState() ) );
...
void FirstScreen::checkPackageState()
{
    int checkCount = 0;
    ...
    if( checkCount != 0 ) {
        emit setPackageState( true );
    } else {
        emit setPackageState( false );
    }
